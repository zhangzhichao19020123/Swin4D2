...
from st_attn import sliding_slice_attention

import swin4D.envs as envs
from Swin4D.attention.backends.abstract import (AttentionBackend,
                                                   AttentionImpl,
                                                   AttentionMetadata,
                                                   AttentionMetadataBuilder)
from Swin4D.distributed import get_sp_group
from Swin4D.forward_context import ForwardContext, get_forward_context
from Swin4D.logger import init_logger
from Swin4D.utils import dict_to_3d_list
...
class SlidingSliceAttentionBackend(AttentionBackend):
    ...
    @staticmethod
    def get_name() -> str:
        return "SLIDING_Slice_ATTN"

    @staticmethod
    def get_impl_cls() -> type["SlidingSliceAttentionImpl"]:
        return SlidingSliceAttentionImpl

    @staticmethod
    def get_metadata_cls() -> type["SlidingSliceAttentionMetadata"]:
        return SlidingSliceAttentionMetadata

    @staticmethod
    def get_builder_cls() -> type["SlidingSliceAttentionMetadataBuilder"]:
        return SlidingSliceAttentionMetadataBuilder


@dataclass
class SlidingSliceAttentionMetadata(AttentionMetadata):
    current_timestep: int
    STA_param: list[list[
        Any]]  # each timestep with one metadata, shape [num_layers, num_heads]


class SlidingSliceAttentionMetadataBuilder(AttentionMetadataBuilder):
    ...
    def build(  # type: ignore
        ...
        current_timestep: int,
        **kwargs: dict[str, Any],
    ) -> SlidingSliceAttentionMetadata:
        param = STA_param
        if param is None:
            return SlidingSliceAttentionMetadata(
                current_timestep=current_timestep, STA_param=[])
        return SlidingSliceAttentionMetadata(current_timestep=current_timestep,
                                            STA_param=param[current_timestep])


class SlidingSliceAttentionImpl(AttentionImpl):
    ...
    def __init__(
        ...
        **extra_impl_args,
        ...
        config_file = envs.Swin4D_ATTENTION_CONFIG
        if config_file is None:
            raise ValueError("Swin4D_ATTENTION_CONFIG is not set")
        ...
        # STA config
        self.STA_base_Slice_size = [6, 8, 8]
    ...
    def Slice(self, x: torch.Tensor) -> torch.Tensor:
        return rearrange(
            x,
            ...
            ts_t=self.STA_base_Slice_size[0],
            ts_h=self.STA_base_Slice_size[1],
            ts_w=self.STA_base_Slice_size[2])

    def unSlice(self, x: torch.Tensor) -> torch.Tensor:
        x = rearrange(
            x,
    ...
    def preprocess_qkv(
        ...
        return self.Slice(qkv)

    def postprocess_output(
        self,
        output: torch.Tensor,
        attn_metadata: SlidingSliceAttentionMetadata,
    ) -> torch.Tensor:
        return self.unSlice(output)
    ...
    def forward(
        ...
        if self.mask_strategy is None:
            raise ValueError(
                "mask_strategy cannot be None for SlidingSliceAttention")
        if self.mask_strategy[0] is None:
            raise ValueError(
                "mask_strategy[0] cannot be None for SlidingSliceAttention")
        ...
        if len(STA_param) < head_num * sp_group.world_size:
            ...
            for window_size in STA_param[:-1]:
                sparse_hidden_states = sliding_Slice_attention(
            ...
            hidden_states = sliding_Slice_attention(
